#!/usr/bin/env bash
# setup.sh — Local setup for eclipse-claw
#
...
build_binaries() {
    ...
    if cargo build --release 2>&1 | tail -5; then
        ...
        ls -lh target/release/eclipse-claw target/release/eclipse-claw-server target/release/eclipse-claw-mcp 2>/dev/null | \
            awk '{printf "    %-20s %s\n", $NF, $5}'
    else
...
# eclipse-claw configuration — generated by setup.sh
...
EOF
    if [[ -n "$proxy_file" ]]; then
        echo "ECLIPSE_CLAW_PROXY_FILE=$proxy_file" >> "$SCRIPT_DIR/.env"
    else
        echo "# ECLIPSE_CLAW_PROXY_FILE=/path/to/proxies.txt" >> "$SCRIPT_DIR/.env"
    fi

    cat >> "$SCRIPT_DIR/.env" <<EOF

# --- Server ---
ECLIPSE_CLAW_PORT=$server_port
...
setup_mcp() {
    ...
    local mcp_binary="$SCRIPT_DIR/target/release/eclipse-claw-mcp"
    if [[ ! -f "$mcp_binary" ]]; then
        warn "eclipse-claw-mcp binary not found. Build first."
        return
    fi

    info "The MCP server lets Claude Desktop use eclipse-claw's tools directly."
    ...
    if ! prompt_yn "Configure MCP server for Claude Desktop?" "y"; then
        ...
        printf '    {"mcpServers": {"eclipse-claw": {"command": "%s"}}}\n' "$mcp_binary"
        return
    fi
    ...
    # Check if eclipse-claw is already configured
    if echo "$existing" | python3 -c "import sys,json; c=json.load(sys.stdin); exit(0 if 'eclipse-claw' in c.get('mcpServers',{}) else 1)" 2>/dev/null; then
        warn "eclipse-claw MCP server already configured in Claude Desktop."
        if ! prompt_yn "Update the path?" "y"; then
            return
        fi
    fi

    # Merge eclipse-claw into mcpServers
    local updated
...
smoke_test() {
    ...
    local eclipse-claw="$SCRIPT_DIR/target/release/eclipse-claw"

    info "Testing extraction..."
    local output
    output=$("$eclipse-claw" https://example.com --format llm 2>/dev/null || echo "FAILED")
    ...
    if curl -sf http://localhost:11434/api/tags &>/dev/null; then
        ...
        local summary
        summary=$("$eclipse-claw" https://example.com --summarize 2>/dev/null || echo "FAILED")
...
print_summary() {
    local eclipse-claw="$SCRIPT_DIR/target/release/eclipse-claw"
    local server="$SCRIPT_DIR/target/release/eclipse-claw-server"
    local mcp="$SCRIPT_DIR/target/release/eclipse-claw-mcp"
    local port
    port=$(grep '^ECLIPSE_CLAW_PORT=' "$SCRIPT_DIR/.env" 2>/dev/null | cut -d= -f2 || echo "3000")
...
main() {
    echo
    printf "${BOLD}${GREEN}  eclipse-claw — Local Setup${RESET}\n"
