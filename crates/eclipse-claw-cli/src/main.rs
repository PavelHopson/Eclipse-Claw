#![allow(dead_code)]
/// CLI entry point -- wires eclipse-claw-core and eclipse-claw-fetch into a single command.
    ...
    match reason {
        ...
        EmptyReason::JsRequired => eprintln!(
            "\x1b[33mwarning:\x1b[0m No content extracted from {url}\n\
             ...
             Use the eclipse-claw Cloud API for JS rendering: https://webclaw.io/pricing"
        ),
        EmptyReason::None => {}
    }
}
...
#[derive(Parser)]
#[command(name = "eclipse-claw", about = "Extract web content for LLMs", version)]
struct Cli {
    ...
    #[arg(short, long, env = "ECLIPSE_CLAW_PROXY")]
    proxy: Option<String>,

    /// File with proxies (host:port:user:pass, one per line). Rotates per request.
    #[arg(long, env = "ECLIPSE_CLAW_PROXY_FILE")]
    ...
    #[arg(long, env = "ECLIPSE_CLAW_WEBHOOK_URL")]
    ...
    #[arg(long, env = "ECLIPSE_CLAW_LLM_PROVIDER")]
    llm_provider: Option<String>,

    /// Override the LLM model name
    #[arg(long, env = "ECLIPSE_CLAW_LLM_MODEL")]
    llm_model: Option<String>,

    /// Override the LLM base URL (Ollama or OpenAI-compatible)
    #[arg(long, env = "ECLIPSE_CLAW_LLM_BASE_URL")]
...
fn init_logging(verbose: bool) {
    let filter = if verbose {
        EnvFilter::new("eclipse-claw=debug")
    } else {
        EnvFilter::try_from_env("ECLIPSE_CLAW_LOG").unwrap_or_else(|_| EnvFilter::new("warn"))
    };
...
fn build_fetch_config(cli: &Cli) -> FetchConfig {
    let (proxy, proxy_pool) = if cli.proxy.is_some() {
        ...
        match eclipse_claw_fetch::parse_proxy_file(path) {
            Ok(pool) => (None, pool),
            Err(e) => {
                eprintln!("warning: {e}");
                (None, Vec::new())
...
async fn run_map(cli: &Cli) -> Result<(), String> {
    ...
    let entries = eclipse_claw_fetch::sitemap::discover(&client, url)
        .await
...
fn fire_webhook(url: &str, payload: &serde_json::Value) {
    ...
    let body = if is_discord {
        ...
        serde_json::json!({
            "embeds": [{
                "title": format!("eclipse-claw: {event}"),
        ...
        let details = serde_json::to_string_pretty(payload).unwrap_or_default();
        serde_json::json!({
            "text": format!("*eclipse-claw: {event}*\n```{details}```")
        })
        .to_string()
    } else {
...
async fn run_watch_single(
    ...
    loop {
        ...
        let diff = eclipse_claw_core::diff::diff(&previous, &current);
...
async fn run_diff(cli: &Cli, snapshot_path: &str) -> Result<(), String> {
    ...
    let diff = eclipse_claw_core::diff::diff(&old, &new_result);
    print_diff_output(&diff, &cli.format);
...
async fn build_llm_provider(cli: &Cli) -> Result<Box<dyn LlmProvider>, String> {
    if let Some(ref name) = cli.llm_provider {
        match name.as_str() {
            "ollama" => {
                let provider = eclipse_claw_llm::providers::ollama::OllamaProvider::new(
            ...
            "openai" => {
                let provider = eclipse_claw_llm::providers::openai::OpenAiProvider::new(
                    None,
                    cli.llm_base_url.clone(),
                    cli.llm_model.clone(),
                )
            ...
            "anthropic" => {
                let provider = eclipse_claw_llm::providers::anthropic::AnthropicProvider::new(
                    None,
                    cli.llm_model.clone(),
                )
...
async fn run_llm(cli: &Cli) -> Result<(), String> {
    ...
    if let Some(ref schema_input) = cli.extract_json {
        ...
        let extracted = eclipse_claw_llm::extract::extract_json(
        ...
        let extracted = eclipse_claw_llm::extract::extract_with_prompt(
            &result.content.plain_text,
            prompt,
            provider.as_ref(),
            model,
        ...
        let summary = eclipse_claw_llm::summarize::summarize(
            &result.content.plain_text,
            Some(sentences),
            provider.as_ref(),
            model,
...
async fn run_batch_llm(cli: &Cli, entries: &[(String, Option<String>)]) -> Result<(), String> {
    ...
    for (i, (url, _)) in entries.iter().enumerate() {
        ...
        let llm_result = if let Some(ref schema) = schema {
            eclipse_claw_llm::extract::extract_json(text, schema, provider.as_ref(), model)
                .await
                .map(LlmOutput::Json)
        } else if let Some(ref prompt) = cli.extract_prompt {
            eclipse_claw_llm::extract::extract_with_prompt(text, prompt, provider.as_ref(), model)
                .await
                .map(LlmOutput::Json)
        } else if let Some(sentences) = cli.summarize {
            eclipse_claw_llm::summarize::summarize(text, Some(sentences), provider.as_ref(), model)
                .await
                .map(LlmOutput::Text)
        } else {
...
async fn run_research(cli: &Cli, query: &str) -> Result<(), String> {
    let api_key = cli
        .api_key
        .as_deref()
        .ok_or("--research requires ECLIPSE_CLAW_API_KEY (set via env or --api-key)")?;
...
mod tests {
    ...
    fn write_to_file_creates_dirs() {
        let dir = std::env::temp_dir().join("eclipse-claw_test_output_dir");
