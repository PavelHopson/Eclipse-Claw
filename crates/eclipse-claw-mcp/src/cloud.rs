...
/// Lightweight client for the eclipse-claw cloud API.
...
impl CloudClient {
    /// Create a new cloud client from ECLIPSE_CLAW_API_KEY env var.
    /// Returns None if the key is not set.
    pub fn from_env() -> Option<Self> {
        let key = std::env::var("ECLIPSE_CLAW_API_KEY").ok()?;
        if key.is_empty() {
            return None;
        }
...
pub fn is_bot_protected(html: &str, headers: &eclipse_claw_fetch::HeaderMap) -> bool {
...
pub enum SmartFetchResult {
    ...
    Local(Box<eclipse_claw_core::ExtractionResult>),
...
pub async fn smart_fetch(
    client: &eclipse_claw_fetch::FetchClient,
    ...
    let options = eclipse_claw_core::ExtractionOptions {
