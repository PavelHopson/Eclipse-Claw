...
/// Automatically detects PDF responses and extracts text via eclipse-claw-pdf.
///
/// Two proxy modes:
...
pub struct BatchExtractResult {
    pub url: String,
    pub result: Result<eclipse_claw_core::ExtractionResult, FetchError>,
}

/// Buffered response that owns its body. Provides the same sync API
/// that eclipse-claw-http::Response used to provide.
struct Response {
    status: u16,
    url: String,
...
impl FetchClient {
    ...
    pub async fn fetch_and_extract(
        ...
        self.fetch_and_extract_with_options(url, &eclipse_claw_core::ExtractionOptions::default())
            .await
    }
    ...
    pub async fn fetch_and_extract_with_options(
        ...
        if is_pdf {
            ...
            let pdf_result = eclipse_claw_pdf::extract_pdf(bytes, self.pdf_mode.clone())?;
    ...
    pub async fn fetch_and_extract_batch(
        ...
        self.fetch_and_extract_batch_with_options(
            urls,
            concurrency,
            &eclipse_claw_core::ExtractionOptions::default(),
        )
        .await
    }
...
/// Convert a eclipse-claw-pdf PdfResult into a eclipse-claw-core ExtractionResult.
fn pdf_to_extraction_result(
    pdf: &eclipse_claw_pdf::PdfResult,
    url: &str,
) -> eclipse_claw_core::ExtractionResult {
    let markdown = eclipse_claw_pdf::to_markdown(pdf);
...
mod tests {
    ...
    fn test_pdf_to_extraction_result() {
        let pdf = eclipse_claw_pdf::PdfResult {
            text: "Hello from PDF.".into(),
            page_count: 2,
            metadata: eclipse_claw_pdf::PdfMetadata {
