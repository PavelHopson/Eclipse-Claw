...
### Fixed
- **MCP research token overflow**: research results are now saved to `~/.eclipse-claw/research/` and the MCP tool returns file paths + findings instead of the full report. Prevents "exceeds maximum allowed tokens" errors in Claude/Cursor.
...
### Added
- **`dirs` dependency** for `~/.eclipse-claw/research/` path resolution.
...
- **MCP extract/summarize cloud fallback**: when no local LLM is available, these tools now fall back to the cloud API instead of erroring. Set `ECLIPSE_CLAW_API_KEY` for automatic fallback.
...
### Changed
- **Replaced custom TLS stack with wreq**: migrated from eclipse-claw-tls (patched rustls/h2/hyper/reqwest) to [wreq](https://github.com/0x676e67/wreq) by [@0x676e67](https://github.com/0x676e67). wreq uses BoringSSL for TLS and the [http2](https://github.com/0x676e67/http2) crate for HTTP/2 fingerprinting — both battle-tested with 60+ browser profiles.
- **Removed all `[patch.crates-io]` entries**: consumers no longer need to patch rustls, h2, hyper, hyper-util, or reqwest. Just depend on eclipse-claw normally.
...
### Removed
- eclipse-claw-tls dependency and all 5 forked crates (eclipse-claw-rustls, eclipse-claw-h2, eclipse-claw-hyper, eclipse-claw-hyper-util, eclipse-claw-reqwest).
...
### Changed
- **Replaced primp with eclipse-claw-tls**: switched to custom TLS fingerprinting stack.
...
### Added
- **Docker image on GHCR**: `docker run ghcr.io/pavelhopson/eclipse-claw` — auto-built on every release
...
- MCP server proxy support via `ECLIPSE_CLAW_PROXY` and `ECLIPSE_CLAW_PROXY_FILE` env vars
...
### Added
- YouTube transcript extraction infrastructure in eclipse-claw-core (caption track parsing, timed text XML parser) — wired up when cloud API launches
...
### Fixed
- MCP server now identifies as `eclipse-claw-mcp` instead of `rmcp` in the MCP handshake
