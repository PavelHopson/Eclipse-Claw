#!/usr/bin/env bash
# deploy/hetzner.sh — One-click Hetzner VPS deployment for eclipse-claw
#
# Creates a Hetzner Cloud VPS with Docker, deploys eclipse-claw + Ollama,
...
SERVER_NAME="eclipse-claw"
REPO_URL="https://github.com/PavelHopson/eclipse-claw.git"
...
# ---------------------------------------------------------------------------
destroy_server() {
    info "Looking for existing eclipse-claw server..."
    local server_id
...
build_cloud_init() {
    ...
    # Build .env content
    local env_content="# eclipse-claw deployment — generated by hetzner.sh
ECLIPSE_CLAW_HOST=0.0.0.0
ECLIPSE_CLAW_PORT=3000
ECLIPSE_CLAW_AUTH_KEY=$auth_key
OLLAMA_HOST=http://ollama:11434
OLLAMA_MODEL=$ollama_model
ECLIPSE_CLAW_LOG=info"
...
- |
  cat > /etc/nginx/sites-available/eclipse-claw <<'NGINX'
  server {
      listen 80;
      server_name $domain;
...
- ln -sf /etc/nginx/sites-available/eclipse-claw /etc/nginx/sites-enabled/eclipse-claw
...
# --- Clone and deploy ---
- git clone $REPO_URL /opt/eclipse-claw
- |
  cat > /opt/eclipse-claw/.env <<'DOTENV'
  $env_content
  DOTENV
  # Remove leading whitespace from heredoc
  sed -i 's/^  //' /opt/eclipse-claw/.env

$nginx_block
# --- Start services ---
- cd /opt/eclipse-claw && docker compose up -d --build
...
- |
  nohup bash -c '
    ...
    for i in \$(seq 1 60); do
      if docker compose -f /opt/eclipse-claw/docker-compose.yml exec -T ollama ollama list &>/dev/null; then
        echo "Ollama ready. Pulling $ollama_model..."
        docker compose -f /opt/eclipse-claw/docker-compose.yml exec -T ollama ollama pull $ollama_model
...
wait_for_docker() {
    ...
    for i in $(seq 1 $max_attempts); do
        ...
        status=$(ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no \
            "root@$ip" "docker ps --filter name=eclipse-claw --format '{{.Status}}' 2>/dev/null | head -1" 2>/dev/null || echo "")
...
create_server() {
    ...
    echo
    printf "${BOLD}${GREEN}  eclipse-claw Hetzner Deploy${RESET}\n"
    printf "${DIM}  One-click VPS deployment for eclipse-claw REST API + Ollama${RESET}\n"
    echo
    ...
    prompt_secret AUTH_KEY "Eclipse Claw auth key" "$generated_auth_key"
    ...
    echo
    printf "  The server is now building eclipse-claw from source.\n"
    ...
    printf "    ssh root@%s 'cd /opt/eclipse-claw && docker compose logs -f'\n" "$server_ip"
    echo
    ...
    printf "    ssh root@%s 'cd /opt/eclipse-claw && docker compose exec ollama ollama pull %s'\n" "$server_ip" "$OLLAMA_MODEL"
    echo
